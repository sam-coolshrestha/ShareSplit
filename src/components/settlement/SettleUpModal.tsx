...
export function SettleUpModal({ groupId, currency, currentUserId, transactions }: SettleUpModalProps) {
  ...
  const getDeepLink = (transaction: ActiveTransaction, app: UpiApp) =>
    buildUpiDeepLink({
      app,
      payeeUpiId,
      payeeName: transaction.toName ?? 'ShareSplit member',
      amount: transaction.amount,
      currency,
      note: `ShareSplit settlement for ${transaction.toName ?? 'member'}`,
    })
  ...
  return (
    <>
            ...
            keyedTransactions.map((transaction) => (
              ...
              >
                <div>
                  ...
                  {statusByKey[transaction.key] ? (
                    <p className="mt-2 text-xs text-muted-light">{statusByKey[transaction.key]}</p>
                  ) : null}
                </div>
      ...
      {activeTransaction ? (
        ...
        >
          <div className="theme-card-priority max-h-[calc(100vh-2rem)] w-full max-w-lg overflow-y-auto bg-surface p-5 sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <Badge variant="primary">UPI</Badge>
                <h3 id="settle-up-title" className="mt-4 font-display text-3xl text-foreground">
                  Settle up
                </h3>
                <p className="mt-2 text-sm leading-6 text-muted-light">
                  {activeTransaction.fromName ?? 'Member'} pays {activeTransaction.toName ?? 'Member'}{' '}
                  {formatCurrency(activeTransaction.amount, currency)}.
                </p>
              </div>
              <button
                type="button"
            ...
            {statusByKey[activeTransaction.key] ? (
              <p className="mt-4 text-sm text-error">{statusByKey[activeTransaction.key]}</p>
            ) : null}
